  ...
  try {
    ...
    try {
      ...
      const { data: topOpportunities, error: fetchError } = await supabase
        ...
        .order('analysis.overall_score', { ascending: false })
        .limit(15);
      ...
      const { count: newEntriesCount } = await supabase
        ...
        .gte('date_discovered', sevenDaysAgo.toISOString());
      ...
      const reportData = {
        ...
        summary: {
          ...
          easiest_builds: (topOpportunities || []).filter(op => 
            op.analysis?.build_complexity === 'Simple'
          ).length
        }
      };
      ...
      const { error: reportError } = await supabase
        ...
        .insert({
          ...
          top_score: topOpportunities?.[0]?.analysis?.overall_score || 0,
          data: reportData
        });
...
function findHighestRevenuePotential(opportunities: any[]): string {
  ...
  opportunities.forEach(op => {
    const revenue = op.analysis?.revenue_potential_monthly || '$500-2000/month';
    const maxRevenue = extractMaxRevenue(revenue);
    
    if (maxRevenue > maxValue) {
      maxValue = maxRevenue;
      highest = revenue;
    }
  });
...
function extractMaxRevenue(revenueString: string): number {
  const match = revenueString.match(/\$(\d+(?:,\d+)*)/g);
  if (match) {
    const numbers = match.map(m => parseInt(m.replace(/[$,]/g, '')));
